...
export default function Dictionary() {
  ...
  return (
    <div className="rounded-md px-6 pt-2 pb-6 flex flex-col gap-2 mb-12">
      <div className="font-semibold">Từ điển</div>
      ...
      {status && result !== undefined && (
        <audio ref={audioRef} src={result?.phonetics[0]?.audio} />
      )}
      {status && (
        <div>
          <div className="mt-2 font-bold text-2xl flex items-center justify-between">
            {result.word}
            <div
              className="rounded-md p-2 hover:bg-secondary cursor-pointer transition duration-300"
          ...
          <div className="flex items-center">
            {result?.phonetic && <div>Pronunciation: {result?.phonetic}</div>}
          </div>
          <div>
                        ...
                        <div key={defindex}>
                          ...
                          {defitem.example && (
                            <>
                              ...
                              <div className="italic">{defitem.example}</div>
                            </>
                          )}
                          {defitem.antonyms.length > 0 && (
                            <>
                              <div className="font mb-1">Antonyms:</div>
                              <div className="italic">
                                {defitem.antonyms &&
                                  defitem.antonyms.map(
                                    (antonym: any, antindex: any) => {
                                      if (
                                        antindex !==
                                        defitem.antonyms.length - 1
                                      )
                                        return (
                                          <span
                                            className="text-sm"
                          ...
                          {defitem.synonyms.length > 0 && (
                            <>
                              <div className="font mb-1">Synonyms:</div>
                              <div className="italic">
                                {defitem.synonyms &&
                                  defitem.synonyms.map(
                                    (synonym: any, synindex: any) => {
                                      if (
                                        synindex !==
                                        defitem.synonyms.length - 1
                                      )
                                        return (
                                          <span
                                            className="text-sm"
